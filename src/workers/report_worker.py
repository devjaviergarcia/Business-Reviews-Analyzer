...
class ReportWorker(QueuedJobWorkerBase):
    ...
    async def _process_job(self, job: dict) -> None:
        ...
        message = f"Report worker handler not implemented for job_type='{job_type}'."
        await self._job_broker.append_event(
            job_id=job_id,
            stage="report_worker_unimplemented",
            message=message,
            data={"queue_name": self.queue_name, "job_type": job_type},
        )
        await self._job_broker.mark_failed(job_id=job_id, error=message)
        LOGGER.warning("Job failed as unimplemented in report worker: job=%s job_type=%s", job_id, job_type)
